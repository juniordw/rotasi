...
// This is the updated enrollCourse function to replace the existing one in CourseDetails.jsx
...
const enrollCourse = async () => {
  try {
    ...
    // Show confirmation dialog before submitting enrollment request
    if (!window.confirm(`Submit enrollment request for "${courseData.courseTitle}"?`)) {
      return;
    }
    ...
    if (data.success) {
      // Instead of redirecting to Stripe, show successful submission message
      toast.success(data.message || 'Enrollment request submitted! Waiting for admin approval.');
      
      // Optionally navigate to enrollments page to see pending status
      if (window.confirm('View your pending enrollments?')) {
    ...
    toast.error(error.response?.data?.message || error.message || 'Failed to submit enrollment request');
  } finally {
